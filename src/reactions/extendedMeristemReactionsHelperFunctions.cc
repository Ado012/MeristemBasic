...
int CRMEventPicker(int site, int eventFlag, std::string action, double eventBegin, double eventEnd,
                   ...
                   int &crmSite, Compartment &compartment, int chrom, int monRecruitLimit, int crmOrMarkerSwitch, int polTimeLimit, int monResidenceTime, int elementsWithAffinity)
{
...
//timer only for monomers with no cooperativity DISABLED condition for timer CHANGE: 111023
                     //condition reenabled and replaced with single cis element requirement 123125 because we want it for some reason
                 if (elementsWithAffinity == 1) //if the crm is a single cis element system start the residence timer
                 {compartment.monResidenceTimer[chrom][site]=monResidenceTime;} //monResidenceTime set to 100

                 else //for all other instances zero out the monomer residence timer
...
//2. Large Overflow handling > 1
    ...
    {
        compartment.stochasticStepOverflow[crmOrMarkerSwitch]=compartment.stochasticStepOverflow[crmOrMarkerSwitch]-timeStep;
...
//4: Overflow output
    overFlowResults1.clv3=clv3Creation; //essentially does nothing? clv3 creation doesn't carry over?
